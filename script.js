...
let blogName = "b0rk.exe projects page";
let authorName = "Jame";
let authorLink = "borkdotexe.com"; // Enter your website, social media, etc. Some way for people to tell you they like your blog! (Leaving it empty is okay too)
...
/*Each time you make a new post, add the filepath here at the top of postsArray.
  ...
  below are expecting it ( 'posts/[type]/YYYY-MM-DD_Title-of-Your-Post.html', ). You can
...
let postsArray = [...techArtPostsArray, ...gameDevPostsArray].sort((newer, older) => {
  let newerPostDate = new Date.parse(newer.split("/")[2].split("_")[0]);
  let olderPostDate = new Date.parse(older.split("/")[2].split("_")[0]);

  return newerPostDate - olderPostDate
});
...
let headerHTML = '<div id="header-links"> \
        <div id="site-links"> \
          <li><a href="' + relativePath + '/index.html">Home</a></li>' + 
        ' <li><a href="' + relativePath + '/techArt.html">Tech Art</a></li>' +
        ' <li><a href="' + relativePath + '/gameDev.html">Gamedev</a></li>' +
        ' <li><a href="' + relativePath + '/about.html">Bio</a></li> \
        </div> \
        <div id="social-links"> \
...
let footerHTML = "<hr><p>" + blogName + " is written by <a href='" + authorLink + "'>" + authorName + "</a> and built with <a href='https://zonelets.net/'>Zonelets</a></p>";
...
function formatPostLink(path) {
  let postDate = path.split("_")[0].split("/")[2]
  let postTitle = path.split("_")[1].split(".")[0].replaceAll("-", " ");

  return '<p><a href="' + relativePath + '/'+ path +'">' + postDate + " \u00BB " + postTitle + '</a></p>'
}

let techArtPostListHTML = "<ul>";
if (techArtPostsArray.length > 0) {
  for ( let i = 0; i < techArtPostsArray.length; i++ ) {
    techArtPostListHTML += formatPostLink(techArtPostsArray[i]);
  }
  techArtPostListHTML += "</ul>";
} else {
  techArtPostListHTML += '<p>Nothing here yet :/</p></ul>'
}

let gameDevPostListHTML = "<ul>";
if (gameDevPostsArray.length > 0) {
  for ( let i = 0; i < gameDevPostsArray.length; i++ ) {
    gameDevPostListHTML += formatPostLink(gameDevPostsArray[i]);
  }
  gameDevPostListHTML += "</ul>";
} else {
  gameDevPostListHTML += '<p>Nothing here yet :/</p></ul>'
}
...
if (postsArray.length > 0) {
  ...
  if ( postsArray.length > recentPostsCutoff ) {
    ...
    recentPostListHTML += '<li>Nothing here yet :/</li></ul>'
}
...
let chanceIndex = Math.floor(Math.random()*10) > 0 ? 1 : 0
let messagesArray = [
  {
    text: "Ahoy! This be me project page!",
    icon: "./assets/images/buccaneer.png"
  },
  {
    text: "Hi, welcome to my project page!",
    icon: "./assets/images/lich_unscaled_no_bg.gif"
  }
]

// let welcomeMessage = messagesArray[chanceIndex];
let welcomeMessage = `<h1>${messagesArray[chanceIndex].text}</h1><img src="${messagesArray[chanceIndex].icon}" style="width:96px;height:96px;">`
//-----------------------------
...
if (document.getElementById("techartpostlistdiv")) {
  document.getElementById("techartpostlistdiv").innerHTML = techArtPostListHTML;
}
if (document.getElementById("gamedevpostlistdiv")) {
  document.getElementById("gamedevpostlistdiv").innerHTML = gameDevPostListHTML;
}
